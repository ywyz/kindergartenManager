...
> 当前检出分支：`feat/agent-foundation`（F005-F008 已固定 GREEN；当前只进入 F009 文档与稳定 RED 门禁）；
...
2. 当前只进入 F009 文档/spec/tasks 与稳定 RED；之后才执行零持久化全矩阵、Linux 浏览器 mock 和仅使用
   应用安全配置凭据的真实模型验收。F009 仍须独立闭合 Review、精确 SHA Quality 与 Issue 证据。
