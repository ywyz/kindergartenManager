...
> 当前检出分支：`feat/agent-foundation`（F003 已固定 GREEN；F004 为 GREEN 候选，交付精确 SHA 以 Git HEAD、远端 ref 和 CI `headSha` 回读为准）；
...
F004 已最小实现冻结 Context 与 tenant+user READ 投影，尚无 Tool 实现、Provider、Runtime、UI 或持久化，
