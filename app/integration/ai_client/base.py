    ...
    *,
    ...
    async def _do_request(client: httpx.AsyncClient) -> dict:
        try:
            ...
            raise AiCallError("AI 请求网络错误") from exc
        ...
        if response.status_code >= 400:
            ...
            raise AiCallError(f"AI 接口返回 HTTP {response.status_code}")

        try:
            body = response.json()
        except Exception as exc:
            logger.info("AI 返回内容解析失败（非 JSON）")
            raise AiParseError("AI 返回内容不是有效 JSON") from exc

        # 提取 choices[0].message.content
        try:
            content_str = body["choices"][0]["message"]["content"]
        except (KeyError, IndexError, TypeError) as exc:
            logger.info("AI 响应结构异常，缺少 choices/message/content")
            raise AiParseError("AI 响应结构异常") from exc

        # 解析 content 为 dict
        try:
            result = json.loads(content_str)
        except json.JSONDecodeError as exc:
            logger.info("AI content 字段不是有效 JSON")
            raise AiParseError("AI content 不是有效 JSON") from exc

        if not isinstance(result, dict):
            logger.info("AI 返回内容不是 JSON 对象")
