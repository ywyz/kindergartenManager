...
本仓库另有两个供 Issue #55 后续业务切片使用的受控、已脱敏种子文件；2026-09-05 的 v2
发布物取代原未跟踪 v1 观察基线，旧 SHA 与 Office 证据随之失效：

- `templates/weekplan.docx`：v2 脱敏 seed 读取到 3 个表格，每个表格为 9 行 × 7 个逻辑列；正文只保留标题、
  空白元数据标签和固定栏目，园所、班级、日期、人员与样例活动内容均已清除；没有显式机器占位符。
- `templates/monthplan.docx`：v2 脱敏 seed 读取到 1 个 8 行 × 4 个逻辑列的表格；正文保留标题和空白班级、执行年月、
  带班/保育老师标签及固定栏目，人员、年月与样例活动内容均已清除；没有显式机器占位符。

两份文件已登记为关闭的 candidate profile，但仍不是 active 运行时模板；不得由空白布局推导业务数据模型。
...
由于当前 v2 `weekplan.docx` 和 `monthplan.docx` 已清除样例值但仍没有显式 token，phase-1 对它们采取
