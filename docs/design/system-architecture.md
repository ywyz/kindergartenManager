...
python -m app.main
  ├─ 解析配置与用户数据目录
  ├─ 不执行 Alembic；schema 由备份门保护的独立任务预先迁移
...
| 边界 | 当前行为 | 风险/要求 |
|---|---|---|
| Alembic 显式迁移 | 先验证已恢复备份证据，再执行 upgrade head | 应用/Bootstrap 零迁移；失败不得启动未知 schema 的发布流程 |
...
- ADR-0007 已冻结显式迁移与已验证备份门；恢复自动化或离线只读模式仍需独立 ADR。
- 下一项功能开发使用哪个固定分支/SHA。
- 逻辑外键是否逐步收紧为数据库外键。
