...
- `test_provider_configuration_and_execution_failures_are_zero_effect()` --uses--> `OpenAICompatibleAgentProvider`  [INFERRED]
  specs/agent-foundation/tests/test_f009_zero_persistence_matrix_red.py → app/integration/ai_client/agent_provider.py
- `test_cancel_during_tool_is_zero_effect()` --uses--> `Permission`  [INFERRED]
  specs/agent-foundation/tests/test_f009_zero_persistence_matrix_red.py → app/service/agent/contracts.py
- `test_coordinator_public_runtime_limits_bound_each_timeout_without_effects()` --uses--> `Permission`  [INFERRED]
  specs/agent-foundation/tests/test_f009_zero_persistence_matrix_red.py → app/service/agent/contracts.py
- `test_each_closed_read_tool_is_zero_effect()` --uses--> `Permission`  [INFERRED]
  specs/agent-foundation/tests/test_f009_zero_persistence_matrix_red.py → app/service/agent/contracts.py
...
Cohesion: 0.09
Nodes (34): create_pending_user(), create_user(), get_user_by_id(), list_users_by_tenant(), AsyncSession, 在指定租户中更新用户启停状态，返回是否更新成功。, 创建待审核用户（is_active=False，role=teacher），用于自助注册流程。      Args:         session: 异步数据, 创建用户并持久化到数据库，返回已持久化的 User 对象。 (+26 more)
...
Cohesion: 0.09
Nodes (25): _make_settings(), 回归测试：config.py 中密钥自动生成与 BOOTSTRAP_ADMIN_* 字段。 核心目标： 1. Settings…, 在受控环境下构造 Settings 实例，不读取磁盘 .env 文件。, 无任何配置时，Settings() 应成功实例化（修复必填字段导致的启动崩溃）。, ENCRYPTION_KEY 为空时应自动生成非空值。, JWT_SECRET 为空时应自动生成非空值。, 已设置的 ENCRYPTION_KEY 不应被自动生成逻辑覆盖。, 首次生成的密钥写入持久化文件，第二次实例化时读回相同值。 (+17 more)
...
Cohesion: 0.15
Nodes (21): ensure_default_user(), AsyncSession, 应用启动引导：确保默认用户存在。  单用户模式下，系统启动时自动在 user 表中创建默认管理员账号。 如果已存在则跳过（幂等）。, 确保默认用户存在，不存在则创建。已存在则跳过。, run_bootstrap(), User, UserRole, has_any_user() (+13 more)
...
Cohesion: 0.25
Nodes (8): Settings, BaseSettings, 并发启动都只能返回同一组已持久化密钥，不能各自成功后丢失一组。, DATABASE_URL 默认空字符串（交由 database.py 降级为 SQLite）。, HOLIDAY_API_URL 有内置默认值，不强制要求 .env 配置。, test_concurrent_settings_initialization_returns_only_persisted_secret_pair(), test_database_url_default_is_empty(), test_holiday_api_url_has_default()
...
- **Why does `save_daily_plan()` connect `Community 3` to `Community 0`, `Community 58`?**
  _High betweenness centrality (0.046) - this node is a cross-community bridge._
- **Why does `TrustedActor` connect `Community 12` to `Community 0`, `Community 1`, `Community 34`, `Community 35`, `Community 4`, `Community 5`, `Community 40`, `Community 11`, `Community 15`, `Community 17`, `Community 52`, `Community 26`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Why does `save_ai_key()` connect `Community 8` to `Community 41`, `Community 82`, `Community 20`, `Community 30`?**
