...
## ⏸ 交接状态（2026-06-21，等待跨电脑手动验收 · 已发布复测标签 v3.1.0-beta2）

> **下次续作：用户将切换电脑测试。阅读本文档后，用户的第一件事是反馈手动测试结果。**
> AI 助手：收到反馈后再决定是否迭代修复，不要在反馈前重复实现已完成的 P8/P8d/P9 与已修复项。

- **代码状态**：P0~P9 + P8d 全部实现并自测通过；并已应用第 1 轮测试反馈修复（见下「测试反馈修复」）；**全量回归通过（466 passed）**；尚未人工复测修复项。
- **复测标签**：`v3.1.0-beta2`（指向提交 `57fd14d`，包含三项修复：节假日判定/超 15 张导入/工作日分布）。
- **复测安装包来源**：GitHub Actions `release.yml` 对应 `v3.1.0-beta2` 的构建产物或同名 prerelease 资产。
- **切换电脑后启动前置**：
  1. 拉取 `dev3.1` 分支最新提交。
  ...
  4. 若仅做安装包复测：直接安装 `v3.1.0-beta2` 产物并启动，无需本地拉代码。
- **验收清单**：见本文件下方「🧑 待手动验收清单（P7/P8 全功能）」。
- **反馈后**：如有问题，按反馈逐项修复并补测；全部通过后可考虑合并分支。

---

## 测试反馈修复 — 第 1 轮（2026-06-21）

针对跨电脑 beta 测试（v3.1.0-beta1）反馈的 3 项问题修复：

1. **自动选工作日未排除节假日（如 5/1）** — 根因：原 `_auto_pick_workdays` 用 `asyncio.gather` 并发 ~15 个 `is_holiday` 请求，timor.tech 返回 **429** 全部降级为「未知」。修复：新增 `holiday_client.get_legal_holidays_in_year(year)`（单次「年」接口请求，按年缓存），`_auto_pick_workdays` 改用之；接口不可用时降级并提示「节假日信息暂不可用，请人工核对」。
2. **上传超 15 张报错** — 改为：≥15 张合法，按文件名排序取前 15（多余忽略并提示）；<15 仍提示「至少 15 张」。
3. **日期总在前三周/总是 1 号** — `pick_three_workdays` 重写为「从本月全部工作日中随机取 3 个不同日期（排除周末+法定节假日），升序返回」，可注入 `rng`；分布于全月、每次结果不同。

**改动文件**：`app/integration/holiday_client/client.py`、`app/service/date_service.py`、`app/ui/pages/one_on_one_listening.py`；测试 `tests/test_date_service.py`、`tests/test_holiday_client.py`、`tests/test_listening_ui_helpers.py`。
**验证**：子集单测 70 passed；实测 `_auto_pick_workdays(2026,5)` 正确排除 5/1、5/4 且分布全月、仅 1 次 HTTP（无 429）。
