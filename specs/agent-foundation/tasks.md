...
权威范围见 [spec.md](spec.md)。下列任务有严格依赖关系；当前只完成 F000-F002，并在 RED 后停止。

| ID | 任务 | 前置 | 当前状态 |
|---|---|---|---|
| F000 | 固定 `feat/agent-foundation` 与 R1 基线 SHA | R1 本地 GREEN | 完成 |
| F001 | 固定 spec、非目标、顺序和停止边界 | F000 | 完成 |
| F002 | 建立契约/关闭 registry 稳定 RED | F001 | RED |
| F003 | 最小实现 contracts 与关闭 registry | F002 + 明确 GREEN 授权 | 未授权 |
...
禁止把 F003-F009 的目录、类、migration、UI 控件或占位实现预建到当前 RED 检查点。
