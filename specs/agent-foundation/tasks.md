...
权威范围见 [spec.md](spec.md)。下列任务有严格依赖关系；F003 已 GREEN，F004 已获单独授权并进入 GREEN 候选，F005 及以后未授权。
...
| F004 | 建立 tenant+user 窄 READ service 投影 | F003 + 明确授权 | GREEN 候选（待 Review/CI） |
...
当前停在 F004 GREEN 候选并进入 Review/验证。禁止预建 F005-F009 的目录、类、migration、UI 控件或占位实现。
