...
| W007 | 在每日计划页增加逐 Patch 确认/过期/失败/对账 UI，保持 Provider READ/DRAFT | W006 GREEN + Review | 第五轮 finding 修复候选已本地 GREEN，尚未取得 fixed-SHA Review 0/0；尚未 push/CI/人工验收/Issue 回写，W008 未进入 |
...
设置/文件/Word/删除/创建写入、长期 Patch 持久化、新 Tool 或多 Agent。完整 W007 RED/GREEN/Review/precheck
SHA、计数与 node hash 只记录在 `specs/agent-write/tests/README.md`。
