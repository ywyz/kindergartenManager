...
short/compact 各1页；long 各2页，明确单页失败。**Windows Word 最短完整/compact单页可行性通过；三份long单页FAIL**。晚间重试已恢复原生画面工具，
九份只读副本实际打开未见修复/兼容警告，12页打印预览全部观察并保存21张原生截图。
原生观察缺项已补齐。用户明确空格字体无需宋体；非空文字宋体已有证据，字体项通过，空格差异不再阻塞。
2026-09-10用户将LibreOffice明确为备用打开，不要求其格式通过；WP-A最短完整Word排版可行性门已满足。正式模板、产品/云端及#77整体未由本轮通过。
...
“可见正文宋体”已确认。用户本轮明确“空格字体不需要是宋体”，因此字体验收PASS；纯空格差异保留为诊断信息，不再作为阻塞项。不声称PDF所有span仅宋体。
COM整表字体/行距读回混合值9999999和空字体名，保留原始读数，不把它作为格式证据。
空格来源未确证，但按用户明确范围无需为本门追查；没有改变字体/字号/行距。

## 九份测量矩阵

下表F=可见文字为嵌入宋体，纯空格含Times New Roman但按用户要求不参与宋体验收，字体PASS；
V=独立逐页审图未见裁切/重叠/左右溢出；C=固定数量全部完整，详见下一节。
短/compact的“可行”指当前合成候选Word/PDF单页排版，不外推产品或全部长度；字体项已闭合。

| 输入 | input SHA256 | Word/PDF页数 | 字体 | 裁切/溢出 | 固定数量 | 本轮结论/理由 | 证据子目录 |
|---|---|---:|---|---|---|---|---|
| normal-five-short.docx | `7eca517011011b5cb7c3b74d38756fdfef6da5f2a7df44503bceefdbd07593c9` | 1/1 | F | V | C | 单页PDF/原生预览可行；字体PASS | normal-five-short/ |
...
| preceding-sunday-six-short.docx | `78b7f8fdab2714a4cb4a5cacfaba55092229462653811ce66503d61e2ff1109a` | 1/1 | F | V | C | 单页PDF/原生预览可行；字体PASS | preceding-sunday-six-short/ |
...
| saturday-six-short.docx | `99bfb2d7be462be4353a5b54affddac60c4bd257040f4a6e237f5bbd58073259` | 1/1 | F | V | C | 单页PDF/原生预览可行；字体PASS | saturday-six-short/ |
...
| 门 | 本轮状态与下一步 |
|---|---|
| Windows Word子门 | 最短完整五/六列及compact可行性PASS；字体和原生观察已闭合；long三份单页FAIL，不能宣称九份全通过 |
| LibreOffice备用打开 | 按用户新口径不设格式阻塞；原16.9pt及long两页诊断保留，Linux格式实测不再要求 |
| WP-A可行性门 | 最短完整Word五/六列证据及契约阶段检查已满足，可收口；不是#77产品验收/Issue关闭，证据尚未提交 |
...
**可以切回Ubuntu继续后续已授权的开发小步，无需等待LibreOffice格式验证。** 切换前保全/传递本轮新证据，
...
> 续接WP-A-Windows-20260910及2026-09-10客户端范围决策：Word为主验收，LibreOffice仅备用打开，空格字体不要求宋体。
> 保全并交接未提交证据；WP-A最短完整五/六列排版可行性已满足，long两页保持负向记录。
> 可以切回Ubuntu，fetch交接分支并保护本机修改。仅按另行授权的WP-C-next-prompt推进共享授权/实际教学日事实小步，先RED再GREEN。
> 不实现正式exporter、模板资格迁移、部署或关闭#77；未来Word验收按实际产品/版本，不能声称所有Word版本已通过。

## GitHub回写

已回写[#77补充评论](https://github.com/ywyz/kindergartenManager/issues/77#issuecomment-5619295160)，合并Word及后续LibreOffice失败结果并读回核对；证据仍本地未提交。
...
PDF的纯空格Times New Roman差异仍保留；后续用户明确空格不要求宋体，解除该阻塞。
...
## 用户字体范围澄清与LibreOffice安装核实

记录时间（UTC）：2026-09-10T13:06:17.135341+00:00。用户明确：“空格字体不需要是宋体。”
据此非空可见文字宋体验收PASS，不再要求空白span也为SimSun；原始PDF、字体读数及此前保守结论快照保留，未重新渲染。

本机 `C:/Program Files/LibreOffice/program/soffice.exe` 与 `soffice.bin` ProductVersion均26.8.0.3；
version.ini buildid `bce0998afefdbc355585ca324285661a2170ba77`。
[官方稳定版页面](https://us.libreoffice.org/download/libreoffice-stable/)列26.8.0并对应26.8.0.3源码包。
安装核实不等于九份样本分页/字体/逐页视觉已通过；可用此安装补做独立Windows LibreOffice实测，无需仅为该验证切换Ubuntu。

## Windows稳定LibreOffice后续实测

[LibreOffice26.8.0.3本轮报告](WP-A-LibreOffice-Windows-20260910.md)：九份均已独立导出，short/compact各1页、long各2页。
可见文字SimSun12/16pt、固定内容保全，但九份所检查连续正文行均16.9pt，不满足源固定20pt；LibreOffice排版FAIL。
先前“已安装但未实测”为当时状态，现由本轮实测替代。此结果不撤销Word的独立测量，也不满足tasks.md规定的Linux LibreOffice门。

## 最新用户决策：Word主验收，LibreOffice备用打开

用户明确存在系统差异，LibreOffice只作备用打开，不要求文档格式；主要使用Microsoft Word系列。
据此取消本周计划LibreOffice（含Linux）格式通过的阻塞要求。旧运行报告的16.9pt/FAIL是原要求下真实测量，完整保留，不改为技术PASS。
当前Word最短完整三种日期形态均一页，非空文字宋体12pt/固定20pt和原生预览已验证，满足WP-A可行性要求；long三份仍两页。
此决策不证明LibreOffice原生UI已人工验收、不证明所有Word系列版本通过，也不授予正式模板/云端产品资格。

客户端口径已[回写#77](https://github.com/ywyz/kindergartenManager/issues/77#issuecomment-5619437453)并读回核对（CRLF/LF归一化后一致）。未关闭Issue，未提交证据。
