> 2026-09-15 修正：读取每日晨间活动、限定同年级数字班名匹配、一键补齐全部可生成缺项及横向主按钮，以 [当前行为契约](autofill-20260914.md#2026-09-15-实际每日格式与一键补全修正) 为准，替代逐栏目生成确认流程。相关本地检查完成；生产部署与真实模型检查必须另记实际结果。

> 当前产品依据：2026-09-14 [本人自动填写与页面精简](autofill-20260914.md) 覆盖本文中冲突的来源选择、晨谈问题、材料强制生成及显式缩减交互。下方阶段记录保留历史，不表示本次已验收或上线。

> 2026-09-14 手动缩短口径（本轮覆盖旧流程）：超页由教师手动缩短，自动缩减完善与验收延期；历史候选23字段不再阻塞本轮手动编辑验证，也不声称已采用历史候选。真实单页检测、超页禁止导出、修改保存后重检保留；浏览器五/六列保存下载与长文手动编辑重检收件尚待本轮实测，不预写通过。页面通过后 Main 可提交分支推送绑定 SHA 查 CI，准备云端测试，生产部署单独执行。范围见[本轮决定](evidence/WP-E-manual-shortening-scope-20260914.md)；下方 WP-E 缩减任务条目按历史时点保留。
...
旧首门通用“各行为都先RED”条目不整体勾选；本轮真实RED和初始通过覆盖分别记录。当前完成说明以[当前状态](current-status.md)为准，不用实现勾选替代全门通过。下方WP-D为任务定义；本轮实现与验证见WP-D补充。WP-E现有本地实现见2026-09-12交付记录，整门未关闭；WP-F未实施。

## WP-D 日期、结构化内容与提示词

2026-09-11：本次授权范围的本地实现和适用验证已完成，逐行证据见[WP-D矩阵](WP-D-completion-contract.md)。固定数量/v3历史兼容、权威日历/每日显示、真实分项生成与确认/CAS闭环均已接线；不代表WP-C/#77整门或外部门PASS。
