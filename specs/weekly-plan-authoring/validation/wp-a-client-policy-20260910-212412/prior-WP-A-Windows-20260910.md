...
| 目标稳定LibreOffice子门 | Windows26.8.0.3九份已实测，但连续正文行距16.9pt不满足20pt，FAIL；Linux稳定版仍缺，历史alpha不替代 |
...
> 续接WP-A-Windows-20260910账本，原生观察与非空文字字体项已闭合，空格字体按用户要求豁免，
> 保留9PDF/12PNG和long单页FAIL。保留纯空格Times New Roman诊断，不改原件、字号或行距。
> 之后使用已核实的Windows LibreOffice26.8.0.3（或另行核实的Ubuntu稳定版）对同hash九份样本独立渲染/逐页Review；补齐后再判断WP-A可行性门，
